    ...
    with gr.Blocks(
        ...
        demo.load(
            ...
            """
                      ...
                      # --------------------------------------------------------
        # WHATSAPP SHARE GATE - FINAL CLEAN VERSION
        # --------------------------------------------------------
        ...
        WHATSAPP_PENDING_KEY = (
            "chetexam_wa_share_pending_v4"
        )

        WHATSAPP_STARTED_KEY = (
            "chetexam_wa_share_started_v4"
        )

        WHATSAPP_INFLIGHT_KEY = (
            "chetexam_wa_share_inflight_v4"
        )
        ...
        # RETURN DETECTOR
        # --------------------------------------------------------
        # Browser-side polling detects return from WhatsApp and clicks
        # the existing hidden sync button. No existing test logic is changed.
        demo.load(
            fn=None,
            inputs=[],
            outputs=[],
            ...
            () => {
                try {
                    if (window.__ceWaReturnListenerV5) {
                        return [];
                    }

                    window.__ceWaReturnListenerV5 = true;

                    const checkWhatsAppReturn = () => {
                        try {
                            if (
